...
class HTTPUtil(object):
    ...
    def __init__(self, max_window=4, max_timeout=16, max_retry=4, proxy='', ssl_validate=False, ssl_obfuscate=False):
        ...
        if self.ssl_validate:
            ...
            self.ssl_context.context.set_verify(OpenSSL.SSL.VERIFY_PEER, lambda c, x, e, d, ok: ok)
    ...
    def create_connection(self, address, timeout=None, source_address=None):
        ...
        for i in range(self.max_retry):
            ...
            for i in range(len(addrs)):
                result = queobj.get()
                if not isinstance(result, (socket.error, ssl.SSLError, OpenSSL.SSL.Error, OSError)):
    ...
    def create_connection_withproxy(self, address, timeout=None, source_address=None, proxy=None):
        ...
        try:
            ...
            except (socket.error, ssl.SSLError, OpenSSL.SSL.Error, OSError):
                pass
...
class LocalProxyServer(SocketServer.ThreadingTCPServer):
    ...
    def handle_error(self, *args):
        ...
        if isinstance(value, (ssl.SSLError, OpenSSL.SSL.Error)) and 'bad write retry' in value.args[1]:
            etype = value = tb = None
        else:
            del etype, value, tb
...
class GAEProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_METHOD_GAE(self):
        ...
        if 'Content-Length' in self.headers:
            ...
            except (EOFError, socket.error, ssl.SSLError, OpenSSL.SSL.Error, OSError) as e:
        ...
        for retry in range(common.FETCHMAX_LOCAL):
            try:
                ...
                elif isinstance(e, (ssl.SSLError, OpenSSL.SSL.Error)) and 'bad write retry' in e.args[-1]:
